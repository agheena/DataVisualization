...
attributes=['Total_disabled_population_Males', 'Total_disabled_population_Females', 'Main_worker_Males', 'Main_worker_Females', 
'Marginal_worker_Less_than_3_months_Males', 'Marginal_worker_Less_than_3months_Females', 
'Marginal_worker_3to6_months_Males', 'Marginal_worker_3to6_months_Females', 'Non_worker_Males',
'Non_worker_Females','Total_Rural_Urban_Rural','Total_Rural_Urban_Total','Total_Rural_Urban_Urban',
...
scaled_df=scaled_df[:1500]
print("len of scaled_df", len(scaled_df))
...
stratasample=int(0.50*data_len)

def randomSample():
    global scaled_df
    global rs_data_frntend
    rs_data_frame=scaled_df.sample(frac=0.50)
...
def findThreeAtributesLoadingsForOriginal():
    ...
    loadings = pd.DataFrame(pca_data.components_.T, columns=['PC1', 'PC2','PC3','PC4','PC5', 'PC6','PC7','PC8'], index=datafrm.columns)
    x1=loadings['PC1'].values
    x2=loadings['PC2'].values
    x3=loadings['PC3'].values
    x4=loadings['PC4'].values
    x5=loadings['PC5'].values
    x6=loadings['PC6'].values
    x7=loadings['PC7'].values
    x8=loadings['PC8'].values
    # print("each pc1 loading",x[2])
    sum_sq=[]
    for i in range(0,len(loadings)):
        sum_sq.append(math.sqrt(math.pow(x1[i],2)+math.pow(x2[i],2)+math.pow(x3[i],2)+math.pow(x4[i],2)+math.pow(x5[i],2)+math.pow(x6[i],2)+math.pow(x7[i],2)+math.pow(x8[i],2)))
...
def findThreeAtributesLoadingsForRandom():
    ...
    loadings = pd.DataFrame(pca_data.components_.T, columns=['PC1', 'PC2','PC3','PC4','PC5', 'PC6','PC7','PC8'], index=random_data.columns)
    x1=loadings['PC1'].values
    x2=loadings['PC2'].values
    x3=loadings['PC3'].values
    x4=loadings['PC4'].values
...
@application.route('/pca_original')
def pca_original():
    global imp_ftrs
    random_data=pd.read_csv(r"flaskDirectory\scaleddf.csv")
    imp_ftrs = imp_ftrs_original
...
@application.route('/mds_euclidean_original')
def mds_euclidean_original():
    data_columns = []
    global imp_ftrs
    imp_ftrs = imp_ftrs_original
    random_data=pd.read_csv(r"flaskDirectory\scaleddf.csv")
    ...
    for i in range(0, 3):
        data_columns[attributes[imp_ftrs[i]]] = random_data[attributes[imp_ftrs[i]]][:samplesize]
    data_columns['clusterid'] = modidata['cluster'][:samplesize]
    print("-----------------------Original DATA COLUMNS FOR MDS EUCLIDEAN--------------------------------")
    print(data_columns.head(samplesize))
    data_columns=data_columns.to_dict(orient='records')
    data_columns=json.dumps(data_columns, indent=2)
    return data_columns
...
@application.route('/mds_correlation_original')
def mds_correlation_original():
    data_columns = []
    global imp_ftrs
    imp_ftrs = imp_ftrs_original
    random_data=pd.read_csv(r"flaskDirectory\scaleddf.csv")
    ...
    for i in range(0, 2):
        data_columns[attributes[imp_ftrs[i]]] = random_data[attributes[imp_ftrs[i]]][:samplesize]
    data_columns['clusterid'] = modidata['cluster'][:samplesize]
    print("-----------------------Original DATA COLUMNS FOR MDS CORRELATION--------------------------------")
    print(data_columns.head(samplesize))
    data_columns=data_columns.to_dict(orient='records')
    data_columns=json.dumps(data_columns, indent=2)
    return data_columns
...
def mds_correlation_adaptive():
    ...
    for index, row in strata_data.iterrows():
        data_columns['clusterid'][x] = row['cluster']
        x = x + 1
    print("------------------Correlation  STRATA DATA COLUMNS---------------------------")
    print(data_columns.head(samplesize))   
    data_columns=data_columns.to_dict(orient='records')
    data_columns=json.dumps(data_columns, indent=2)
    return data_columns

@application.route('/scatter_matrix_original')
def scatter_matrix_original():
    ...
    for i in range(0, 3):
        data_columns[attributes[imp_ftrs[i]]] = random_data[attributes[imp_ftrs[i]]][:samplesize]
    data_columns['clusterid'] = modidata['cluster'][:samplesize]
    print("------------------SCATTER PLOT MATRIX Original DATA COLUMNS---------------------------")
    print(data_columns.head(samplesize))
    data_columns=data_columns.to_dict(orient='records')
    data_columns=json.dumps(data_columns, indent=2)
    return data_columns
...
def scatter_matrix_random():
    ...
    for i in range(0, 3):
        # data_columns[attributes[li[i]]] = random_data[attributes[li[i]]][:samplesize]
        data_columns[attributes[imp_ftrs[i]]] = random_data[attributes[imp_ftrs[i]]][:samplesize]
    data_columns['clusterid'] = modidata['cluster'][:samplesize]
...
# mds_correlation_random()
mds_correlation_adaptive()
# pca_original()
# mds_euclidean_original()
# mds_correlation_original()
# scatter_matrix_original()
